...
export default function SignUpForm({ additionalData }: SignUpFormProps) {
  const FormSchema = z.object({
    ...
    isTeacher: z.boolean(),
    ...additionalData.attributes.fields.reduce((acc, field) => {
      ...
      <InformationBox title='Dados'>
        <FormProvider {...registerForm}>
          ...
          >
            ...
            {additionalData && additionalData.attributes.fields.length > 0 && (
              <>
                <h2 className='text-center font-bold text-3xl col-span-3 mt-4'>
                  Informações adicionais
                </h2>
                {additionalData.attributes.fields.map((field) => (
                  <Form.Field size={field.options.size} key={field.id}>
